...
\begin{frame}{Contexto e Objetivo}
  ...
  \begin{block}{Objetivo Principal}
    Comparar a acurácia (viés) e a precisão (Erro Quadrático Médio - RMSE) da REML contra os métodos baseados no variograma (WLS e OLS) sob diferentes tamanhos amostrais pequenos a médios ($n \in \{50, 100, 200\}$).
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Parâmetros Verdadeiros}
        \begin{itemize}
          \item \textbf{Efeito Pepita ($\tau^2$):} $0.20$
          \item \textbf{Patamar ($\sigma^2$):} $1.00$
          \item \textbf{Alcance ($\phi$):} $20.0$
...
\begin{frame}{Conclusões}
  \begin{block}{Principais Descobertas}
    \begin{itemize}
      ...
      \item Portanto, para amostras pequenas a médias ($n \le 200$), priorizar a estimação por REML devido à eficiência estatística superior. Para aplicações de grande escala ($n > 1000$), pode ser necessário utilizar WLS devido ao custo computacional.
    \end{itemize}
  \end{block}
\end{frame}
