...
\begin{frame}{Contexto \& Objetivo Estratégico}
  \begin{block}{Motivação}
    \begin{itemize}
      \item A \textbf{Krigagem Ordinária} fornece predições espaciais ótimas (BLUP) cujos pesos dependem da função de covariância espacial.
      \item Tradicionalmente, os parâmetros (\textbf{Nugget $\tau^2$}, \textbf{Partial Sill $\sigma^2$} e \textbf{Alcance $\phi$}) são estimados por variograma empírico (\textbf{WLS/OLS}) ou por verossimilhança (\textbf{REML}).
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Objetivo Principal}
    Comparar a acurácia e o erro quadrático médio (RMSE) da \textbf{Máxima Verossimilhança Restrita (REML)} contra os métodos baseados no variograma empírico (\textbf{WLS} e \textbf{OLS}) em diferentes tamanhos amostrais ($n \in \{50, 100, 200\}$).
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Parâmetros Verdadeiros}
        \begin{itemize}
          \item \textbf{Nugget ($\tau^2$):} $0.20$ (Ruído microescala)
...
\begin{frame}{Resultados Numéricos Comparativos ($N = 100$)}
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{ccccccc}
      ...
      \midrule
      $n = 50$ & \textbf{\color{remlgreen}REML} & Nugget ($\tau^2$) & 0.20 & 0.186 & -0.014 & \textbf{0.177} \\
      ...
      \midrule
      $n = 100$ & \textbf{\color{remlgreen}REML} & Sill ($\sigma^2$) & 1.00 & 1.145 & +0.145 & \textbf{0.578} \\
      $n = 100$ & OLS & Sill ($\sigma^2$) & 1.00 & 1.376 & +0.376 & 1.133 \\
      $n = 100$ & \textbf{\color{remlgreen}REML} & Alcance ($\phi$)   & 20.0 & 26.96 & +6.96  & \textbf{26.80} \\
      \midrule
      $n = 200$ & \textbf{\color{remlgreen}REML} & Nugget ($\tau^2$) & 0.20 & 0.192 & -0.008 & \textbf{0.068} \\
      $n = 200$ & \textbf{\color{remlgreen}REML} & Sill ($\sigma^2$) & 1.00 & 1.002 & +0.002 & \textbf{0.357} \\
...
\begin{frame}{Conclusões \& Recomendações Gerenciais}
  \begin{block}{Principais Descobertas}
    \begin{enumerate}
      ...
      \item \textbf{Mitigação de Superestimativa do Alcance:} Para $n = 50$, o OLS superestimou o alcance em $+118\%$ (RMSE 54.6), enquanto o REML manteve viés controlado em $+12\%$ (RMSE 16.8).
      \item \textbf{Precisão do Nugget:} O REML estima o Nugget ($\tau^2$) com RMSE reduzido em mais de $70\%$ em relação ao WLS para $n = 50$.
    \end{enumerate}
  \end{block}
  ...
  \begin{alertblock}{Recomendações Práticas para Aplicação}
    \begin{itemize}
      \item \textbf{Amorstras Pequenas a Médias ($n \le 200$):} Priorizar a estimação por **REML** devido à eficiência estatística superior.
      \item \textbf{Aplicações de Grande Escala ($n > 1000$):} Utilizar WLS quando o custo computacional da inversão matricial $O(n^3)$ do REML for proibitivo.
    \end{itemize}
  \end{alertblock}
\end{frame}
